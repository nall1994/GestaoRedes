...
public class SnmpConfigurator {
    ...
    public static final String F_TLS_VERSION = "s{=TLSv1.2}<TLSv1[\\.1|\\.2]?[,TLSv1[\\.1|\\.2]?]*>";
    ...
    public static final String F_DTLS_VERSION = "s{=DTLSv1.2}<DTLSv1[\\.0|\\.2]?[,DTLSv1[\\.0|\\.2]?]*>";
    ...
    public void configure(Session snmp, USM usm, MessageDispatcher messageDispatcher, Map settings) {
        if (usm != null) {
    ...
    private void configureEngine(Session snmp, USM usm, Map settings) {
        int engineBoots = 0;
        Integer bc =
    ...
    public Address getBindAddress(Map settings) {
