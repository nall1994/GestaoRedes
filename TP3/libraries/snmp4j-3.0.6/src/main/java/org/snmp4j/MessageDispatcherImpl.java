...
public class MessageDispatcherImpl implements MessageDispatcher {
    ...
    private final Map<Class<?>, List<TransportMapping<? extends Address>>> transportMappings =
    ...
    public Collection<TransportMapping<? extends Address>> getTransportMappings() {
        ArrayList<TransportMapping<? extends Address>> l = new ArrayList<>(transportMappings.size());
        synchronized (transportMappings) {
            for (List<TransportMapping<? extends Address>> tm : transportMappings.values()) {
                l.addAll(tm);
            }
        }
        return l;
     ...
     */
    @SuppressWarnings("unchecked")
    protected <A extends Address> void sendMessage(TransportMapping<A> transport, Address destAddress, byte[] message,
        ...
        if (transport != null) {
            if (destAddress instanceof GenericAddress) {
                ...
                transport.sendMessage((A)destAddress, message, tmStateReference, timeoutMillis, maxRetries);
            }
        } else {
    ...
    public TransportMapping<?> getTransport(Address destAddress) {
        ...
        do {
            List<TransportMapping<? extends Address>> l = transportMappings.get(addressClass);
            if ((l != null) && (l.size() > 0)) {
                return l.get(0);
            }
        }
     ...
     */
    protected void dispatchMessage(TransportMapping<?> sourceTransport, MessageProcessingModel mp,
                                   Address incomingAddress, BERInputStream wholeMessage,
        ...
        if (status == SnmpConstants.SNMP_ERROR_SUCCESS) {
            ...
            switch (status) {
                ...
                case SnmpConstants.SNMP_MP_USM_ERROR: {
                    @SuppressWarnings("unchecked")
                    AuthenticationFailureEvent<? super Address> event =
                            new AuthenticationFailureEvent<Address>(this, incomingAddress,
                                    (TransportMapping<? super Address>) sourceTransport, status, wholeMessage);
     ...
     *         the event to fire.
     */
    protected void fireAuthenticationFailure(AuthenticationFailureEvent<? super Address> event) {
