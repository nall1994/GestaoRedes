...
public class DefaultTlsTmSecurityCallback implements TlsTmSecurityCallback<X509Certificate> {
    ...
    private OctetString mapCertToTSN(X509Certificate cert,
        ...
        switch (mappingType) {
            ...
            case SANIpAddress: {
                Object entry = TLSTMUtil.getSubjAltName(cert.getSubjectAlternativeNames(), 7);
                if (entry != null) {
     ...
     *         fingerprint is specified by the <code>type</code> parameter.
     * @param type
     ...
     *
     * @return the removed mapping or <code>null</code> if there is no such mapping.
     */
