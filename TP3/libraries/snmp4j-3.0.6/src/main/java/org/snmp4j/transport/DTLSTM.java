...
public class DTLSTM extends DefaultUdpTransportMapping implements X509TlsTransportMappingConfig,
    ...
    private Map<InetSocketAddress, SocketEntry> sockets = new Hashtable<>();
...
//    @Override
     ...
     * {@link org.snmp4j.transport.tls.DefaultSSLEngineConfiguration} which will then become the configured SSL engine configurator. This method
    ...
    protected class TlsTrustManager implements X509TrustManager {
        ...
        public void checkClientTrusted(X509Certificate[] x509Certificates, String s)
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("Checking client trusted with "+Arrays.asList(x509Certificates)+" certs and: "+s);
            }
            if ((tmStateReference != null) && (tmStateReference.getCertifiedIdentity() != null)) {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug("Fingerprint "+fingerprint.toHexString()+" does not match");
                }
            }
            else if (logger.isDebugEnabled()) {
                logger.debug("Could not verify fingerprint because tmStateReference is null or "+"" +
                        "does not have a certified identity");
            }
            ...
            if (!useClientMode && (callback != null)) {
                if (callback.isClientCertificateAccepted(x509Certificates[0])) {
                    ...
                    logger.debug("Security callback "+callback+" does not trust end cert "+x509Certificates[0]);
                }
            }
            try {
